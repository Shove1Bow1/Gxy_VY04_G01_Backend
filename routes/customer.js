        ...
        if (!req.body.CUS_PASSWORD) {
            ...
            con.query(
                ...
                + "' and CUSTOMER_SECURITY.CUSTOMER_ID=CUSTOMER_INFO.CUSTOMER_ID;", (err, result) => {
                    if (err) res.end();
                    if (!result[0]) { res.send("Please reenter your Account"); }
                    else {
...
function handleServerUserInfo(CUSTOMER_INFO_PACKAGE, req, res, callback) {
    console.log("confirm");
    const CUSTOMER_PACKAGE_ALL=[{
